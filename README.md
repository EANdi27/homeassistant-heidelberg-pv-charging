...
5. Füge die Sensoren aus der Datei 'sensoren.yaml' und Automationen aus der Datei `automations.yaml` in dein Home Assistant ein. Passe die PV-Sensoren an deine eigene PV-Anlage an (Tipp: Nutze eine KI wie ChatGPT oder Claude, um die Sensor-Namen schnell auf dein System umzuschreiben).
6. Passe die Datei 'komplett.yaml' an Deine Bedürfnisse an. Flash den ESP nach jeder Änderung
7. Viel Erfolg!
